...
class MixinModel(AbstractMixin):
    ...
    async def _get_from_storage(self, collection: str, data: dict):
        response = await self.storage.get_one(collection, data)
        if not response:
            return None
        return response

    async def _get_from_pagination(
        self, collection: str, data: dict, _from: int, _to: int
    ) -> list | None:
        response = await self.storage.find_by_pagination(collection, data, _from, _to)
        if not response:
            return None
        return response

    async def _update_to_storage(self, collection: str, data: dict, doc_id: ObjectId):
    ...
    async def _get_all_from_storage(self, collection: str, data: dict):
        response = await self.storage.get_all(collection, data)
        if not response:
            return None
        return response
